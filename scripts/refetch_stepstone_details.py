...
async def _mcp_refetch(targets: list[dict], config: dict, uid: str) -> dict[str, dict]:
    ...
    for (gid, zip_code), group_jobs in groups.items():
        ...
        # Establish ONE session for this (group, zip) pair — re-running search_jobs per
        # batch triggers Stepstone rate-limiting and returns 0 results.
        group_session_id = ""
        try:
            async with streamablehttp_client(server_url) as (read, write, _):
                async with ClientSession(read, write) as session:
                    ...
                    try:
                        ...
                        print(f"  [WARN] initial search_jobs timeout for {gid} @ {zip_code}",
                              file=sys.stderr)
                        continue
                    fresh_text = res.content[0].text or "" if res.content else ""
                    group_session_id, job_count, _ = parse_search_response(fresh_text)
                    if not group_session_id:
                        print(f"  [WARN] no session_id for {gid} @ {zip_code}", file=sys.stderr)
                        continue
                    print(f"  Session for {gid} @ {zip_code}: "
                          f"{group_session_id[:8]}... server_count={job_count}")
        except Exception as e:
            print(f"  [ERROR] initial search failed for {gid} @ {zip_code}: {e}", file=sys.stderr)
            continue
            ...
            try:
                async with streamablehttp_client(server_url) as (read, write, _):
                    async with ClientSession(read, write) as session:
                        ...
                        # Reuse the session established above — avoids extra search_jobs calls
                        fresh_id = group_session_id
                        print(f"    Batch {batch_start // _DETAIL_BATCH_SIZE + 1}: "
                              f"session={fresh_id[:8]}... (reused)")
