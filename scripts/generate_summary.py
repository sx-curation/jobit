...
def cross_source_dedup(records: list[dict]) -> list[dict]:
    ...
    for group in key_map.values():
        ...
        if linkedin_recs and stepstone_recs:
            ...
            kept = max(group, key=lambda r: r.get("match_score") or r.get("scoring", {}).get("match_score", 0))
...
def build_markdown(records: list[dict]) -> str:
    ...
    # 按 match_score 降序排序
    records_sorted = sorted(records, key=lambda r: r.get("match_score") or r.get("scoring", {}).get("match_score", 0), reverse=True)
    ...
    for i, rec in enumerate(records_sorted, 1):
        score = rec.get("match_score") or rec.get("scoring", {}).get("match_score", 0)
        ...
        # Support both old schema (recommended_emphasis / missing_skills)
        # and new schema (strengths / required_skills[candidate_has=false])
        emphasis = rec.get("recommended_emphasis") or [
            s if isinstance(s, str) else s.get("point", str(s))
            for s in (rec.get("strengths") or [])
        ]
        raw_missing = rec.get("missing_skills") or []
        if not raw_missing:
            raw_missing = [
                s for s in (rec.get("required_skills") or [])
                if isinstance(s, dict) and not s.get("candidate_has", True)
            ]
        missing = [m if isinstance(m, str) else m.get("skill", str(m)) for m in raw_missing]
