...
def _prune_seen_jobs(h: dict, days: int = 14) -> None:
    ...
    h["seen_jobs"] = {
        ...
        if v.get("last_seen", v.get("first_seen", "9999-99-99")) >= cutoff
    }
...
def build_group_meta(config: dict) -> dict[str, dict]:
    ...
    for g in config["job_search"].get("keyword_groups", []):
        gid = g["group_id"]
        cv_parsed = (
            str(OUTPUT_DIR / f"cv_parsed_{gid}.json")
            if OUTPUT_DIR is not None
            else f"output/cv_parsed_{gid}.json"
        )
        meta[gid] = {
...
def dedup_and_sort(batch_id: str, config: dict) -> dict:
    ...
    global_max_display = config["job_search"].get("max_display", 30)
    group_max_display = {
        g["group_id"]: g.get("max_display", global_max_display)
        for g in config.get("job_search", {}).get("keyword_groups", [])
    }
    ...
    for job in raw_results:
        ...
        if jid in seen:
            ...
            if ex["company"].lower() == co and ex["title"].lower() == title:
                if not ex.get("needs_refetch", False):      # ← 豁免 needs_refetch
                    seen[jid]["last_seen"] = today
                    skipped += 1
                    continue
                # needs_refetch=True → 放行，让本批次重新 fetch + 分析

        # 规则 B：(公司, 职位) 已见过（跨批次跨 URL 去重，主要针对 Stepstone）
        if co and title and (co, title) in seen_ct:
            orig_jid = seen_ct[(co, title)]
            if orig_jid not in seen or not seen[orig_jid].get("needs_refetch", False):
                if orig_jid in seen:
                    seen[orig_jid]["last_seen"] = today
                skipped += 1
                continue
            # 原 jid 有 needs_refetch=True → 放行

        seen_ct[(co, title)] = jid  # 批次内去重
        new_jobs.append(job)

    # ── Step 2：预评分 + 标记低分（per-group，不跨组混用）
    global_min_score = config.get("job_search", {}).get("min_score_for_analysis", 20)
    group_min_scores = {
        g["group_id"]: g.get("min_score_for_analysis", global_min_score)
        for g in config.get("job_search", {}).get("keyword_groups", [])
    }
    for job in new_jobs:
        ...
        job["skip_analysis"] = job["match_score_preview"] < group_min_scores.get(gid, global_min_score)
    ...
    # ── Step 4：截取（per-group: 单组批次用该组 max_display，多组混合用全局值）
    batch_groups = set(j.get("_group_id", "") for j in new_jobs)
    if len(batch_groups) == 1:
        eff_max_display = group_max_display.get(next(iter(batch_groups)), global_max_display)
    else:
        eff_max_display = global_max_display
    display_jobs = new_jobs[:eff_max_display]
    hidden_count = max(0, len(new_jobs) - eff_max_display)
    ...
    for job in new_jobs:
        ...
        seen[jid] = {
            ...
            "needs_refetch":       bool(job.get("_needs_refetch", False)),
        }
