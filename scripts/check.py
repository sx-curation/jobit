...
def main():
    ...
    parser.add_argument("--source", default=None, choices=["linkedin", "stepstone"],
                        help="仅检查指定搜索源（linkedin：跳过 Stepstone 检查）")
    ...
    # --source linkedin: skip Stepstone check entirely (LinkedIn-only search)
    skip_stepstone = (getattr(args, "source", None) == "linkedin")
    if args.phase2:
        check_linkedin_mcp(args.verbose)
        if cfg and not skip_stepstone:
            check_stepstone_server(cfg, args.verbose, strict=True)
        elif cfg and skip_stepstone:
