...
def _run(args):
    ...
    for group in groups:
                ...
                with _seen_lock:
                    for e in entries:
                        ...
                        if jid not in seen_job_ids:
                            seen_job_ids[jid] = False          # 待 fetch 后更新
                            group_seen_ids.add(jid)
                            group_entries.append(e)
                            new_count += 1
                        elif not seen_job_ids[jid]:            # 已见但 detail 无效 → 允许本 group re-fetch
                            group_entries.append(e)
                            new_count += 1
        ...
        # 构建反向索引（在 merge loop 之前）
        _jid_to_idx: dict[str, int] = {
            j["job_id"]: i for i, j in enumerate(all_jobs_with_details)
        }
        ...
        for entry in group_entries:
            ...
            job = {**detail, **{k: v for k, v in entry.items() if k.startswith("_")}}
            primary_jobs.append(job)

            valid = not job.get("_needs_refetch", True)
            if jid in seen_job_ids and not seen_job_ids[jid] and valid:
                # re-fetch 成功：原地替换旧的 bad-detail 条目
                if jid in _jid_to_idx:
                    all_jobs_with_details[_jid_to_idx[jid]] = job
                seen_job_ids[jid] = True
            elif jid not in seen_job_ids:
                seen_job_ids[jid] = valid
        ...
        # extend 时跳过已原地替换的条目（避免重复）
        for job in primary_jobs:
            jid = job.get("job_id", "")
            if jid in _jid_to_idx and seen_job_ids.get(jid, False):
                pass  # 已替换，跳过
            else:
        ...
        if high_score_count < FALLBACK_THRESHOLD:
                ...
                for future in as_completed(futures):
                    ...
                    with _seen_lock:
                        for e in entries:
                            jid = e["job_id"]
                            if jid not in seen_job_ids:
                                seen_job_ids[jid] = False
                                fallback_entries.append(e)
                                new_count += 1
                            elif not seen_job_ids[jid]:            # bad-detail 允许 fallback 补足
                                fallback_entries.append(e)
                                new_count += 1
            ...
            if fallback_entries:
                ...
                for entry in fallback_entries:
                    ...
                    merged_job = {**detail, **{k: v for k, v in entry.items() if k.startswith("_")}}
                    all_jobs_with_details.append(merged_job)
                    valid = not merged_job.get("_needs_refetch", True)
                    seen_job_ids[jid] = valid or seen_job_ids.get(jid, False)
                PARTIAL_SAVE.write_text(
                    json.dumps(all_jobs_with_details, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
